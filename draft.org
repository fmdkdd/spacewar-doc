...
#+Title: Building a multiplayer space action game in HTML5
...
#+LATEX_HEADER: \definecolor{racoon}{hsb}{0.1, 0.3, 0.5}

#+LATEX_HEADER: \lstset{tabsize=3, breaklines=false, columns=fullflexible}
#+LATEX_HEADER: \lstset{frame=leftline, framerule=2pt, rulecolor=\color{racoon}}

#+LATEX_HEADER: \lstnewenvironment{javascript}
#+LATEX_HEADER: {\lstset{ language={C}, keywords={var, function} }}{}

#+LATEX_HEADER: \lstnewenvironment{coffeescript}
#+LATEX_HEADER: {\lstset{ language={Ruby} }}{}
...
* Other works
  ...
  conceived as a demonstration of Ruby on Rails and HTML5 capabilities
  ...
  sound. Although the HTML5 standard introduces an audio API, browser
  implementations might have been lacking when Rawkets was
  conceived.]. Like Lazeroids, Rawkets primary purpose is to
  demonstrate the capabilities of HTML5 and Node in the fun setting of
  a multiplayer space fighting game.

* The technology behind Spacewar

** HTML5 standard
   The Web Hypertext Application Technology Working Group (WHATWG) has
   been spearheading web technology innovation since 2004, branching
   off the World Wide Web Consortium (W3C) to draft the HTML5
   specification, when the W3C was solely focused on XHTML 2.0. In
   2009, the W3C abandoned XHTML 2.0 and reunited with the WHATWG to
   formalize HTML5. While the specification has yet to be finished at
   the time of this writing, some parts are stable and already
   implemented in most web browsers. The canvas element Spacewar uses
   for drawing is such a part, but the WebSocket protocol is still
...
*** Canvas
    The canvas HTML element \cite{canvas} is an important part of
    HTML5. It was initially created by Apple for Safari and the Mac OS
    ...
    the GPU driver directly can lead to crashes unrelated with the
...
*** WebSocket
    ...
    polling and HTTP streaming. Both are described RFC 6202
    ...
    it immediately sends a new request for the next update. In HTTP
    ...
    have disadvantages over a straightforward TCP socket. When
    ...
    establishing a TCP connection between a client and a server,
    allowing two-way message passing after an HTTP handshake
    part. Using the HTTP request/response model, both client and
    server acknowledge of an "HTTP upgrade" to use the WebSocket
    ...
    TCP connection, the WebSocket protocol provides:
    ...
    connection. Messages can either be sent as UTF-8 strings or as raw
    binary.
...
** Node
   ...
   however that this PHP implementation of WebSocket was meant for
   trivial demonstrations purposes, but not for real applications.
   ...
   to other protocols (like AJAX, HTTP long polling or HTTP streaming)
...
*** Rendering performance
    ...
    #+CAPTION: Pre-rendered drawing of a planet saved as a sprite
    #+CAPTION: image to be stamped on the canvas to save time in the
    #+CAPTION: rendering loop.
...
** Server side
   ...
   order. First, a HTTP server is started to serve the client
   files. The second step is to bind the WebSocket to the HTTP server,
   listening for a HTTP upgrade request initiated by the client
...
*** The update loop
    ...
    Let's have a look at the (abridged) update loop:
    ...
    angle will be decreased [fn:: The origin of the HTML canvas
...
* Future improvements
  ...
  client can start the game. An active game would have a unique URL
  attached to it. The client who created the game should forward this
  URL to all the players he wants to spar with. Upon accessing this
  URL, those players would immediately join the action.
  ...
  more thought, but at this time we envision to have a delegated HTTP
  ...
  depending on the access URL. This program would thus act similarly
  to a reverse proxy.
